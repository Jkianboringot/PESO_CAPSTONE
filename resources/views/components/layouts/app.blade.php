...
<div class="flex h-screen overflow-hidden">
    ...
    <!-- ===================== SIDEBAR ===================== -->
    <aside id="sidebar"
           class="flex-shrink-0 w-64 flex flex-col overflow-y-auto"
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5">
            ...
            <a href="{{ route('dashboard') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-chart-pie w-4 text-center {{ request()->routeIs('dashboard') ? 'text-blue-400' : 'text-slate-400' }}"></i>
                <span>Dashboard</span>
            </a>
            ...
            <a href="{{ route('applicants') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-users w-4 text-center {{ request()->routeIs('applicants') ? 'text-blue-400' : 'text-slate-400' }}"></i>
            ...
            <a href="{{ route('analytics') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-chart-bar w-4 text-center {{ request()->routeIs('analytics') ? 'text-blue-400' : 'text-slate-400' }}"></i>
            ...
            <a href="{{ route('reports') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-file-alt w-4 text-center {{ request()->routeIs('reports') ? 'text-blue-400' : 'text-slate-400' }}"></i>
            ...
            <a href="{{ route('duplicates') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-copy w-4 text-center {{ request()->routeIs('duplicates') ? 'text-blue-400' : 'text-slate-400' }}"></i>
            ...
            <a href="{{ route('skills-gap') }}"
               class="nav-link-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ...
                <i class="fas fa-search w-4 text-center {{ request()->routeIs('skills-gap') ? 'text-blue-400' : 'text-slate-400' }}"></i>
                <span>Skills Gap Analysis</span>
            </a>

            <!-- Divider -->
            <div class="my-3 border-t border-white/10"></div>
            <p class="px-3 text-xs font-semibold uppercase tracking-widest mb-1" style="color: #475569;">Administration</p>
        ...
        <div class="px-4 py-4 border-t border-white/10">
            <div class="flex items-center gap-2">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs" style="color:#64748b;">{{ auth()->user()->role?->name ?? 'Administrator' }}</div>
                </div>
            </div>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="flex-shrink-0 flex items-center justify-between px-6 py-0 border-b"
            ...
            <div class="flex items-center gap-3">
                <button class="text-slate-500 hover:text-slate-700 lg:hidden" onclick="document.getElementById('sidebar').classList.toggle('hidden')">
